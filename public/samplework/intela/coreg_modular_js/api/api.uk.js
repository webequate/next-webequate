...
var coregApi = {
    ...
    validatePhone: function() {
        ...
        var url_phone = "/api/v1/isvalidphone/uk/" + $("#phone").val() + "/ws/?apikey=" + $.olp_var.apikey;
    ...
    validateEmail: function() {
        ...
        var url_email = "/api/v1/isvalid/" + $("#email").val() + "/" + $("#firstName").val() + "/" + $("#lastName").val() + "/ws/" + "?apikey=" + $.olp_var.apikey;
        ...
        $.getJSON(url_email, function(result) {
            ...
            if (result.data == "PASS") {
                $("#row_email .valid-fail, #row_firstName .valid-fail, #row_lastName .valid-fail").remove();
                $("#row_email, #row_firstName, #row_lastName").append("<div class='valid-true'></div>");
                ...
                $("#row_email .valid-fail, #row_firstName .valid-fail, #row_lastName .valid-fail").remove();
                $("#row_email, #row_firstName, #row_lastName").append("<div class='valid-fail'></div>");
            }
        });
    },
    ...
    validateZip: function() {
        ...
        var url_zip = "/api/v1/getaddresses/" + $("#zip").val() + "/?apikey=" + $.olp_var.apikey;
        ...
        $.getJSON(url_zip, function(result) {
            ...
            if (result.data == 'false' || result.data.addresses.length == 0) {
                ...
                for (var i = 0; i < result.data.addresses.length; i++) {
                    ...
                    $('#addressList').append('<li><a href="javascript:void(0);" onclick="if ($.olp_var.addressSelected == false) { $.olp_var.addressSelected = true; $.fancybox.close(); coregApi.selectAddress(\'' + a.town + '\',\'' + a.house + '\',\'' + a.houseNumber + '\',\'' + a.houseName + '\',\'' + a.street + '\',\'' + a.address1 + '\',\'' + a.address2 + '\'); }">' + a.address1 + caption + a.town + '</a></li>');
                }
                $('#addressList').append('<li><a href="javascript:void(0);" onclick="$.fancybox.close(); coregUtility.addContinueClickHandler(); $(\'#zip\').attr(\'disabled\',\'\');">My address is not listed here</a></li>');
                $.fancybox.hideLoading();
                $("a#fancylink").click();
                $("#row_zip .valid-fail").remove();
    ...
    selectAddress: function(town, house, houseNumber, houseName, street, address1, address2) {
    ...
    createLead: function() {
        ...
        $.ajax( {
            ...
            success: function (result) {
                ...
                if (result.status === "OK" && $('#nextbutton').css('display') == 'none') {
                ...
                } else if (result.status === "OK" && $('#nextbutton').css('display') == 'block') {
                    coregOffers.submitHiddenMiniOffers();
