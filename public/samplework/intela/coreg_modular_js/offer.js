...
var coregOffers = {
  populateMiniOffers: function (filterOffers) {
    if (filterOffers) {
      var url_offers =
        "/api/v1/path/getoffers/0/?apikey=intela-9a0843d7-da06-466e-b908-36ae8eb5bdf3&pubId=" +
        pub_id +
        "&subPubId=" +
        subpub_id;
    } else {
      var url_offers =
        "/api/v1/path/" +
        path_name +
        "/page/0/?apikey=intela-9a0843d7-da06-466e-b908-36ae8eb5bdf3&pubId=" +
        pub_id +
        "&subPubId=" +
        subpub_id;
    }
        ...
        for (var i = 0; i < result.data.offers.length; i++) {
          ...
          if (offer.isHidden) {
            ...
            markup +=
              ...
              offer["small_image_background-image"] +
              '" />';
            markup +=
              "<p>" + coregUtility.decodeTwo(offer.description) + "</p>";
            markup +=
              '<input type="radio" name="mini_' +
              offer.id +
              '" style="display:none" />';
            if (offer.isPreSelected) {
              markup +=
                '<input type="radio" name="mini_' +
                offer.id +
                '" value="yes" class="radioField" checked="checked" />' +
                $.i18n.prop("label.yes") +
                ",";
            } else {
              markup +=
  ...
  submitHiddenMiniOffers: function () {
    ...
    $("input[type='hidden'][name^='mini_']").each(function () {
      ...
      var url =
        "/api/v1/acceptoffer/?apikey=intela-9a0843d7-da06-466e-b908-36ae8eb5bdf3";
      var data = {
        offerId: offerid,
        pathId: path_id,
  ...
  submitVisibleMiniOffers: function () {
    ...
    $("input[type='radio'][name^='mini_'][value='yes']:checked").each(
      function () {
        var offerid = $(this).attr("name");
        offerid = offerid.replace("mini_", "");
        var url =
    ...
    $("input[type='radio'][name^='mini_'][value='no']:checked").each(
      function () {
        var offerid = $(this).attr("name");
        offerid = offerid.replace("mini_", "");
        var url =
          "/api/v1/rejectoffer/?apikey=intela-9a0843d7-da06-466e-b908-36ae8eb5bdf3";
        var data = {
          offerId: offerid,
          pathId: path_id,
          leadId: $.olp_var.leadId,
